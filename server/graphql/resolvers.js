...
export const resolvers = {
  ...
  Mutation: {
    ...
    deleteProject: async (_, { _id }) => {
      ...
      return `El proyecto ${deletedProject} ha sido eliminado`;
    },
    deleteTask: async (_, { _id }) => {
      const deletedTask = await Task.findByIdAndDelete(_id);
      if (!deletedTask) throw new Error("Task not found");

      return `La tarea ${deletedTask} ha sido eliminada`;
    },
  ...
  Project: {
    task: async (parent) => await Task.find({ projectId: parent._id }),
  },
